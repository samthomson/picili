...
export class SettingsComponent implements OnInit {
  ...
  onDisconnectDropbox() {
	// confirm
	if (
		confirm('Disconnect Dropbox? Picili will loose all your dropbox files.')
	) {
		// start loading
		this.bDisconnectingDropbox = true;

		this.httpService.disconnectDropbox().subscribe(data => {
  ...
  onPrivacyCheckChange(bNewValue) {
	...
	this.httpService.updatePrivacy(bNewValue).subscribe(data => {
		this.sStartedSaving = '';
